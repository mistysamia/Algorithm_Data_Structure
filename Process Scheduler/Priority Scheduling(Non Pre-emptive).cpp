...
int main()
{
    ...
    ll cpu[n + 5], arrival[n + 5], priority[n + 5];
    ...
    while (!process.empty() || !track.empty())
    {
        ...
        if (!track.empty())
        {
            pii time = track.top();
            track.pop();
            if (curTime <= time.ff.ff.ff)
                curTime = time.ff.ff.ff;

            ganttLine += "| (" + to_string(curTime) + ")----P" + to_string(time.ss + 1) + "----(";
            waitTime = abs(curTime - time.ff.ff.ff);
            turnTime = waitTime + time.ff.ff.ss;
            avgWait += waitTime;
            avgTurn += turnTime;
            curTime += time.ff.ff.ss;
